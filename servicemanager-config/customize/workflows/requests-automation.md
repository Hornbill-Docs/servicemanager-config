...
Use the Authorization Decision node to mark a request with the current status of an authorization. This works independently of any particular type of authorization. Setting the authorization decision will typically follow the outcome of an [authorization](/esp-config/automation/authorization); however, it can also be used in any circumstance where some form of acceptance is required.
...
* Before an authorization, the Authorization Decision can be set to **Pending**.
...
* **Add Linked Request**. This task is designed to link a single request to the request that is running the workflow.  The Request ID of the request to be linked must be available to the workflow. This automates the manual [Link Request Action](/servicemanager-user-guide/service-portfolio/requests/link-action). 
...
* **Source**.
* **Status**. Update the status to New, Open, On-hold, Resolved, Closed, or Canceled.  Setting a request to Canceled will also cancel the workflow and the workflow will not continue past this point.
* **Timeline**.
